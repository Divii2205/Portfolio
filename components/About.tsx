...
export default function About() {
    ...
    // map tailwind font-size → bubble radius
    ...
    const [nodes, setNodes] = useState<RoleNode[]>(() =>
        roles.map((role, i) => {
            const base = sizeToBaseRadius[role.size] ?? 50
            const padding = 9                      // 🔹 small extra padding
            const extra = Math.max(0, role.text.length - 10) * 2 // more letters → slightly bigger bubble
            const radius = base + extra + padding
    ...
    useEffect(() => {
        ...
        const sim = forceSimulation<RoleNode>(nodes)
            ...
            // pulls everything into a soft ball
            .force('radial', forceRadial(140, width / 2, height / 2).strength(0.5))
            // gentle repulsion so they don’t overlap too much
            .force('charge', forceManyBody().strength(-20))
            // 🔹 a bit more than +4 so circles don't touch
            .force('collision', forceCollide<RoleNode>().radius(d => d.radius + 6))
            .alpha(0.9)
            .alphaDecay(0.03)
            .on('tick', () => {
                setNodes([...sim.nodes()])
    ...
    // mouse interaction: gentle repel only, no pulling everything together
    const handleMouseMove = (e: MouseEvent<HTMLDivElement>) => {
        ...
        const maxDist = 150      // how far the cursor influence reaches
        const strength = 2       // how strong the push is
        ...
        >
            <div className="max-w-7xl mx-auto relative z-10 w-full">
                <div className="grid lg:grid-cols-2 gap-16 items-center">
                    {/* LEFT: LeetCode-style bubble cluster */}
                    <motion.div
                        ref={containerRef}
                    ...
                    {/* RIGHT: your existing text content */}
                    <motion.div
                    ...
                    >
                        ...
                        <p className="glass-effect p-8 rounded-3xl border border-white/5 relative overflow-hidden group hover:border-purple-500/30 transition-colors">
                            ...
                            <br /><br />
                            I enjoy learning, trying new things, and
                            working with people who share the same passion for making a difference.
                            I hope to do something meaningful for society and create a
                            <span className="text-gradient font-semibold"> POSITIVE IMPACT</span> through my work and ideas.
                        </p>
